...
### Linux Primary Clipboard Auto-Copy
On Linux, selecting text normally overwrites the middle-click PRIMARY clipboard buffer. YuzuFox disables this (`clipboard.autocopy = false`) to prevent accidental overwrites of text you explicitly copied with `Ctrl + C`. To re-enable traditional X11 auto-copy:
```js
